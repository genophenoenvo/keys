...
```
```{r}
options(verbose = TRUE, progress = TRUE)
```
## Build a project 
...
```{r}
setwd('/home/rstudio/NEON_SRER_DP1.30003.001_2019/2019/FullSite/D14/2019_SRER_3/L1/DiscreteLidar/ClassifiedPointCloud/')
```

Create a catalog of the entire flight (at the directory level)

```{r}
project <- catalog("/home/rstudio/NEON_SRER_DP1.30003.001_2019/2019/FullSite/D14/2019_SRER_3/L1/DiscreteLidar/ClassifiedPointCloud/")
plot(project)                   
```
...
```{r}
options(buffer = 15, multicore = 16, tiling_size = 100)
```
...
```{r}
output = catalog_apply(project, tree_area)
